      ...
      for (int y = 0; y < ROWS; y += TILE) {
	    for (int x = 0; x < COLS; x += TILE) {
		  ...
		  for (int px = x; px < x + TILE; px++) {
			...
			equalH = equalH & (M[y][x] == M[y + TILE - 1][px]);
		  }
		  ...
		  if (equalH && equalV && M[y][x] == maxiter) {
			...
			for (int py = y; py < y + TILE; py++) {
				tareador_start_task("py_fill_all");
			      for (int px = x; px < x + TILE; px++) {
				    M[py][px] = M[y][x];
				    if (output2display) {
			      ...
			      tareador_end_task("py_fill_all");
            }
		  } else {
			// computation of tile
			for (int py = y; py < y + TILE; py++) {
				tareador_start_task("py_comput_tile");
			      for (int px = x; px < x + TILE; px++) {
			      ...
			      tareador_end_task("py_comput_tile");
            }
		  }
